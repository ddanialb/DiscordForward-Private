class ProtectionManager {
  ...
  async handleVoiceStateUpdate(oldState, newState) {
    ...
    if (
      ...
      if (oldState.channelId && !newState.channelId) {
        console.log(
          `🚨 Protected user ${oldState.member.user.tag} was disconnected from voice channel!`
        );
  ...
  async handleProtectedUserDisconnected(oldState) {
    try {
      ...
      console.log(
        `🔍 Checking audit logs for user ${protectedUserId} in ${guild.name}...`
      );
      ...
      try {
        ...
        console.log(
          `📋 Found ${auditLogs.entries.size} MEMBER_DISCONNECT entries`
        );
        ...
        const disconnectLogs = auditLogs.entries.filter((entry) => {
          ...
          if (isDirectTargetMatch) {
            ...
            console.log(
              `🎯 Timing match: ${
                entry.executor.tag
              } disconnected someone (likely protected user) ${Math.floor(
        ...
        if (disconnectLogs.length > 0) {
          ...
          const sortedLogs = disconnectLogs.sort(
            (a, b) => b.createdTimestamp - a.createdTimestamp
          );
          disconnectLog = sortedLogs[0];

          console.log(
            `🎯 TARGET IDENTIFIED: ${disconnectLog.executor.tag} (${disconnectLog.executor.id})`
          );
        }
        ...
        if (!disconnectLog && auditLogs.entries.size > 0) {
          ...
          if (
            mostRecentEntry &&
            mostRecentEntry.executor &&
            Date.now() - mostRecentEntry.createdTimestamp < 15000
          ) {
            disconnectLog = mostRecentEntry;
            console.log(
              `🎯 USING MOST RECENT: ${disconnectLog.executor.tag} (${disconnectLog.executor.id})`
            );
          }
        }
      } catch (error) {
        console.error(
          `❌ Error fetching MEMBER_DISCONNECT audit logs:`,
          error.message
        );
      }
      ...
      if (
        ...
        disconnectLog.executor.id
      ) {
        const executor = disconnectLog.executor;
        console.log(
          `🎯 CONFIRMED: ${executor.tag} (${executor.id}) disconnected protected user in ${guild.name}`
        );

        // Skip if the executor is the protected user themselves (self-disconnect)
        if (executor.id === protectedUserId) {
          console.log(
            "ℹ️ Protected user disconnected themselves - no punishment needed"
          );
          return;
        }
        ...
        if (!member) {
          try {
            ...
            console.log(
              `⚠️ Could not fetch member ${executor.tag} - they may have left the server`
            );
            return;
          }
        }
        ...
        console.log(
          `⚖️ Taking action against ${executor.tag} for disconnecting protected user...`
        );
        await this.punishUser(
          member,
          `Disconnected protected user: ${oldState.member.user.tag}`
        );
      } else {
        console.log(
          "⚠️ Could not identify who disconnected the protected user via audit logs"
        );
      }
    } catch (error) {
  ...
  async punishUser(member, reason) {
    try {
      ...
      if (member.voice.channel) {
        console.log(
          `🎯 ${member.user.tag} is in voice channel: ${member.voice.channel.name}`
        );
        ...
        console.log(
          `✅ Successfully disconnected ${member.user.tag} from voice channel`
        );

        console.log(`🎉 Punishment completed for ${member.user.tag}!`);
      } else {
        console.log(
          `⚠️ ${member.user.tag} is not in voice channel, cannot punish`
        );
      }
    } catch (error) {
