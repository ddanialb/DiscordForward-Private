...
class VoiceManager {
  ...
  setupEventListeners() {
    ...
    this.client.once("ready", () => {
      ...
      console.log(
        `💾 Stored original display name: ${this.originalBotDisplayName}`
      );

      if (this.config.voiceChannelId) {
        console.log(
          "🎵 Voice channel ID provided - will maintain 24/7 voice presence"
        );
  ...
  async joinVoiceChannel() {
    ...
    try {
      const guild = this.client.guilds.cache.find((g) =>
        g.channels.cache.has(this.config.voiceChannelId)
      );
      if (!guild) {
      ...
      this.voiceConnection.on(VoiceConnectionStatus.Ready, () => {
        console.log(
          "✅ Successfully connected to voice channel for 24/7 presence"
        );
      });
  ...
  async handleVoiceStateUpdate(oldState, newState) {
    ...
    if (newState.member && this.movedUsers.has(newState.member.id)) {
      if (newState.channelId === this.config.voiceChannelId) {
        console.log(
          `🔄 User ${newState.member.user.tag} rejoined their designated voice channel`
        );
  ...
  async handleBotVoiceChange(oldState, newState) {
    ...
    if (
      ...
      console.log(
        `🔄 Bot was moved to channel ${
          newState.channel?.name || newState.channelId
        }, returning to designated channel in 3 seconds...`
      );
      ...
      setTimeout(async () => {
        try {
          ...
          console.error(
            "❌ Failed to rejoin designated channel:",
            error.message
          );
        }
      }, 3000);
    ...
    if (oldState.channelId && !newState.channelId) {
      console.log(
        `🔄 Bot was disconnected from voice, reconnecting to designated channel...`
      );
      setTimeout(async () => {
        try {
          await this.rejoinDesignatedChannel();
          ...
          console.error(
            "❌ Failed to reconnect to designated channel:",
            error.message
          );
        }
      }, 2000);
  ...
  async handleUserRejoin(member) {
    try {
      ...
      await member.setNickname(
        "Engadr Move Nade Pesar",
        "User rejoined after being moved"
      );
      console.log(
        `🏷️ Changed display name for ${member.user.tag} to "Engadr Move Nade Pesar"`
      );
      ...
      const timer = setTimeout(async () => {
        try {
          ...
          await member.setNickname(
            originalName,
            "Restoring original display name"
          );
          console.log(
            `🏷️ Restored original display name for ${member.user.tag}`
          );
          ...
          console.error(
            `❌ Failed to restore display name for ${member.user.tag}:`,
            error.message
          );
        }
      }, 5 * 60 * 1000); // 5 minutes
      ...
      console.error(
        `❌ Failed to change display name for ${member.user.tag}:`,
        error.message
      );
    }
  }
  ...
  async changeBotDisplayName() {
    try {
      ...
      console.log(
        `🏷️ Changed global display name to "Enghadr Move Nade Pesar" (visible in ALL servers)`
      );
      ...
      this.botDisplayNameTimer = setTimeout(async () => {
        try {
          ...
          console.log(
            `🏷️ Restored original global display name: ${this.originalBotDisplayName}`
          );
          this.botDisplayNameTimer = null;
        } catch (error) {
          console.error(
            `❌ Failed to restore original display name:`,
            error.message
          );
        }
      }, 5 * 60 * 1000); // 5 minutes
