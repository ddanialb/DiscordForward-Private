class EnhancedProtection {
    ...
    async handleProtectedUserMuted(oldState, newState) {
        try {
            ...
            if (muteLog && muteLog.executor.id !== protectedUserId) {
                ...
                await this.findAndPunishUser(executor.id, 'mute', oldState.member.user.tag);
            }
        } catch (error) {
    ...
    async handleProtectedUserDeafened(oldState, newState) {
        try {
            ...
            if (deafenLog && deafenLog.executor.id !== protectedUserId) {
                ...
                // Try to find and punish the executor in any voice channel across all servers
                await this.findAndPunishUser(executor.id, 'deafen', oldState.member.user.tag);
            }
        } catch (error) {
    ...
    async handleProtectedUserMoved(oldState, newState) {
        try {
            ...
            if (moveLog && moveLog.executor.id !== protectedUserId) {
                ...
                console.log(`🎯 CONFIRMED: ${executor.tag} moved protected user to ${targetChannel?.name}`);
                
                // Move the executor to the same channel where they moved the protected user
                await this.findAndMoveUser(executor.id, targetChannel, oldState.member.user.tag);
            }
        } catch (error) {
    ...
    async findAndPunishUser(userId, action, protectedUserTag) {
        console.log(`🔍 Searching for user ${userId} across all servers to apply ${action}...`);
        ...
        for (const guild of this.client.guilds.cache.values()) {
            try {
                ...
                if (fetchedMember && fetchedMember.voice.channel) {
                    console.log(`🎯 Found ${fetchedMember.user.tag} in voice channel: ${fetchedMember.voice.channel.name} (${guild.name})`);
                    
                    try {
                        if (action === 'mute') {
                            await fetchedMember.voice.setMute(true, `Muted protected user: ${protectedUserTag}`);
                            console.log(`✅ Successfully muted ${fetchedMember.user.tag} in ${guild.name}`);
                        } else if (action === 'deafen') {
                            await fetchedMember.voice.setDeaf(true, `Deafened protected user: ${protectedUserTag}`);
                            console.log(`✅ Successfully deafened ${fetchedMember.user.tag} in ${guild.name}`);
                        }
                        return; // Found and punished, stop searching
                    } catch (error) {
                        console.log(`⚠️ Failed to ${action} ${fetchedMember.user.tag} in ${guild.name}: ${error.message}`);
                    }
                }
            } catch (error) {
        ...
        console.log(`⚠️ Could not find user ${userId} in any voice channel across all servers`);
    }

    async findAndMoveUser(userId, targetChannel, protectedUserTag) {
        console.log(`🔍 Searching for user ${userId} across all servers to move them to ${targetChannel?.name}...`);
                ...
                if (fetchedMember && fetchedMember.voice.channel) {
                    ...
                    try {
                        if (targetChannel && guild.id === targetChannel.guild.id) {
                            ...
                            await fetchedMember.voice.setChannel(targetChannel, `Moved protected user: ${protectedUserTag}`);
                            console.log(`✅ Successfully moved ${fetchedMember.user.tag} to ${targetChannel.name}`);
                        } else {
                            // If target channel is not available, disconnect them
                            await fetchedMember.voice.disconnect(`Moved protected user: ${protectedUserTag}`);
                            console.log(`✅ Successfully disconnected ${fetchedMember.user.tag} as retaliation`);
                        }
                        return; // Found and moved, stop searching
                    } catch (error) {
                        console.log(`⚠️ Failed to move ${fetchedMember.user.tag}: ${error.message}`);
                    }
                }
            } catch (error) {
                // Skip guilds where bot doesn't have access
