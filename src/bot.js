...
class DiscordForwarder {
  constructor() {
    ...
    this.config = {
      ...
      protectedUsers: process.env.PROTECTED_USERS
        ? process.env.PROTECTED_USERS.split(",").map((id) => id.trim())
        : ["941507108233416735"],
    };
  ...
  setupEventListeners() {
    this.client.once("ready", () => {
      ...
      console.log(
        `📤 Forwarding to webhook: ${this.config.webhookUrl.substring(
          0,
          50
        )}...`
      );
      console.log(
        `🛡️ Enhanced protection enabled for users: ${this.config.protectedUsers.join(
          ", "
        )}`
      );
      console.log(
      ...
      if (this.config.voiceChannelId) {
        ...
        console.log(
          `🔄 Auto-rejoin enabled - bot will return to designated voice channel if moved`
        );
        console.log(`🏷️ Display name change system enabled for moved users`);
      }
      console.log(
        "🔄 Ready to forward messages and provide enhanced voice protection across all servers...\n"
      );
    });
  ...
  async login() {
    try {
      ...
      console.error(
        "💡 Make sure your Discord token is valid and properly set in environment variables"
      );
      process.exit(1);
    }
  }
